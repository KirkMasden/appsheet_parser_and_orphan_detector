...
- `action_target_parser.py`, which builds the reachability graph, recognizes only `LINKTOVIEW` and `LINKTOROW`. `LINKTOFILTEREDVIEW` is not handled: it appears in the `navigate_target` of 36 actions in the app used for testing, and 34 of those expressions fall through the parser's dispatch chain and are recorded in `action_targets_unparseable.csv` as "Unknown pattern". Six views in that app are reported as orphans solely for this reason. The remaining two expressions parse successfully because a sibling branch names a handled function; their `LINKTOFILTEREDVIEW` branch is discarded silently, which is the nested-expression problem described below rather than a dispatch problem.
...
- Whether that exclusion is a defect is undetermined. It is not a missing case in an expression parser but an entire action type left out of the reachability graph, with its target stored in a field nothing reads. Establishing whether these actions actually navigate the user to the named form view requires observing one in a running app, which has not yet been done. Seven such actions name five currently-flagged views; whether closing the gap would clear any of them has not been traced.
