...
### Slice-sourced views mis-expand `**auto**` when a slice mixes auto-assigned and explicit actions — a live reachability defect (2026-09-05)

- `views_parser.py` (the `available_actions` block, ~lines 851–869) populates a view's `available_actions` from the source slice's `slice_actions` field when the view's `data_source` is a slice. AppSheet writes an auto-assigned slice's action list as the literal string `**auto**`, and the parser correctly expands that to the full source-table action list — but only when it is the *sole* entry: the guard tests `len(available_actions) == 1 and available_actions[0] == '**auto**'`. A slice whose action list mixes `**auto**` with explicitly named actions falls through it.
- **What the mixed case produces.** `available_actions` ends up holding the explicitly named actions *plus* the literal string `**auto**` carried through as though it were an action name, and *missing* every auto-assigned action the source table contributes. The error runs in both directions at once — one spurious member, an unknown number of real ones absent.
- **Live, not latent.** Farmy has 4 mixed slices; 2 of them — `MyPlants with GPS Location` and `AmmendmentPrep_CurrentAmendment` — are the `data_source` of real views, 5 views in total. Kankaku has 0 mixed slices (its 57 slices are 10 auto-only, 2 empty, 45 explicit-only), so this is invisible in that app.
- **Why it is a reachability defect and not a labelling one.** `available_actions` feeds `actions_orphan_detector.py`'s reachability test directly (`action_name_lower not in available_actions: continue`). An auto-assigned action on the source table that is absent from a mixed slice's explicit list is therefore wrongly excluded from that view's available-action set, and can be reported as an action orphan when it is reachable — a false positive in the output, not a mislabeled report.
- **Weight, stated because the survey's earlier findings were of a different kind.** This is the FIRST reachability-affecting finding the component survey has produced. The two findings in the 2026-09-05 presentation-layer working note (view display names absent from the export; column display names being formulas over live state) were both purely labelling — they change what a report can *call* a thing, not what the tool concludes about it. This one changes a conclusion. Do not read the three as a series of equivalent findings.
- **Fix is output-affecting.** `available_actions` is a persisted column of `appsheet_views.csv`, so any correction changes parse output and requires a predicted diff plus a full re-parse against both saved reference parses, not an edit-and-commit. Useful prediction for that diff: Kankaku should be byte-identical throughout (0 mixed slices); all movement should be confined to Farmy, and within Farmy to the 5 views sourced from those 2 slices, plus whatever `potential_action_orphans.csv` rows those views were suppressing.
- Found 2026-09-05 by a read-only slice component survey, the third specimen (after views and columns) of the survey method described in the presentation-layer working note. Not fixed; no code changed.
...
- Recorded so a future reader does not repeat the survey. The defects it turned up are the three entries directly above.
