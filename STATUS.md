...
### The "third app" this file measures against is shimodabt's, shared 2025-12 — identity and paths, no defect
...
### `navigation_edge_generator.py` joined actions to targets by name alone, with no table boundary, fabricating edges in every app where an action name recurs across tables
...
- Found 2026-09-06 while diagnosing the three Farmy phantom names above; Kankaku could not have surfaced it, since its action names do not recur across tables this way.
- **Fixed 2026-09-06.** `_target_row_belongs_to_view(target_row, view)` added to `navigation_edge_generator.py` and called at all three sites that read `self.targets_by_action`: `process_view`, `process_event_actions`, and `process_group_action` (which looks up by the CHILD action's name and threads the same view object through every recursion level). A target row is eligible only when its own `source_table` equals the view's resolved table, obtained via `get_view_table()` — never `data_source`, which holds the slice name for slice-backed views and would have wrongly refused 314 Kankaku / 958 Farmy / 158 third-app edges.
- **Two permit rules, both "cannot determine defaults to permit, not refuse."** (1) A target row whose own `source_table` is empty or literally `Unknown` is permitted — 0 such rows in any of the three apps' `action_targets.csv`, but the rule is defensive, not dead code, since nothing guarantees that stays true. (2) A view whose resolved table matches neither a known table (`self.columns_by_table`) nor a known slice (`self.slices`) is also permitted, added after three Farmy edges were wrongly refused by the first cut of this fix — see "`views_parser.py`'s `data_source` loses double spaces..." below. Permits granted under rule 2: 0 Kankaku, 3 Farmy, 0 third app — exactly the three views that entry names.
- **Final row counts.** Kankaku 585, unchanged. Farmy 2063 → 1760 (−303). Third app 626 → 382 (−244). Every other output file is logically identical to its pre-fix parse in all three apps, and the reachable set is unchanged — 131 / 178 / 57 — with no view newly flagged and no view newly cleared.
- **The predicted −47/−7 was superseded, and the prediction was the thing at fault, not the fix.** That figure counted only edges landing on a destination the source view's own table cannot reach. It missed the larger case: where several tables' variants of one action name resolve to the SAME destination, the view received one duplicate row per table. `ActivityAmendment_Detail` carried five identical rows to `Plants without DB reference_Detail`, of which four were fabricated by this defect and one was its own. Farmy: 50 rows on keys that disappear entirely plus 256 on duplicate collapse = 306 removed before the view-table guard restored 3 of them, net 303. Third app: 7 and 237 = 244, none restored — none of the third app's removed edges involved an unresolvable view table.
- **A clean diff in every other output file is expected and is NOT the evidence this fix worked.** Every affected target happened to retain enough legitimate paths in these three graphs; that is a property of today's data, not of the fix. The evidence is the edge removal in `navigation_edges.csv`, each removal traced individually to a table mismatch, and the three restorations traced individually to the view-table guard.
- **One path lengthening:** Farmy's `MyPlantsReadOnly_Detail` shortest path grows from 1 hop to 2, a root having reached it directly only via a fabricated edge. Far under the 6-edge cap, so no new disagreement between `view_orphan_detector.py` and `view_dependency_analyzer.py` today — but the mechanism behind the depth-cap margin concern is observed here rather than hypothetical.
- The three phantom names are NOT affected: `phantom_view_reference_detector.py` reads `action_targets.csv`, never `navigation_edges.csv`.

### `views_parser.py`'s `data_source` loses a genuine double space wherever a table or slice name contains one, because it is read from a browser copy-paste capture, not from the HTML export

- **Traced to source, not guessed.** `data_source` (and therefore `source_table`, via `resolve_table_reference`) is populated entirely from `self.view_to_table_map`, built by a line-by-line text parser (`views_parser.py` lines ~60–220) reading `views1.txt`/`views2.txt` — the "select-all-and-paste from the legacy editor" captures `README.md` documents as the setup step, not from `Application Documentation.html`. The deciding line: `info['data_source'] = text_mapping.get('data_source', '')` (line 622); nothing in this module ever reads a data-source field out of the HTML for a view.
- **The cause: a real double space in the app's own naming, present in the HTML, lost in the paste capture.** Confirmed byte-for-byte in Farmy's export. The HTML's own "Slice Name" field (from `Application Documentation.html`, which `slice_parser.py` reads directly) contains two literal space characters — e.g. `'NurseryDetails Inline Table  NurseryCreatingDashboard'` — carried faithfully into `appsheet_slices.csv`. The same name in `views1.txt`'s `Data:` line reads `'NurseryDetails Inline Table NurseryCreatingDashboard'` — one space. `views_parser.py`'s string-splitting logic extracts exactly what `views1.txt` contains; it is not misreading its input. The information is lost before it reaches this module, in the browser's own rendering of the live editor page: two consecutive space characters in HTML source collapse to one when rendered, and a select-all-and-copy captures the rendered (collapsed) text, not the source markup. `Application Documentation.html`, by contrast, is a static export whose "Slice Name" cell carries the raw string uncollapsed.
- **Four Farmy views confirmed affected**, all traced to a real slice whose true name (per `appsheet_slices.csv`, from the HTML) contains a double space that the view's `data_source` (from `views1.txt`/`views2.txt`) does not:
  - `NurseryDetails Inline Table` — `data_source` `NurseryDetails Inline Table NurseryCreatingDashboard`; real slice `NurseryDetails Inline Table  NurseryCreatingDashboard`.
  - `NurseryDetails_Inline Seeds_Detail` — `data_source` identical to the view's own name; real slice `NurseryDetails_Inline  Seeds_Detail` (double space after `Inline`).
  - `Reminders_Inline Date Description` — `data_source` `Reminders Inline Date Description`; real slice `Reminders Inline  Date Description` (double space after `Inline`).
  - `Not successfully transplanted` — carries no actions, so it never reached a wrong verdict, but shows the mechanism most clearly: `data_source` `DELETE ActivityTransplant items not successfully added to veggie beds`; real slice `DELETE ActivityTransplant  items not successfully added to veggie beds`.
  - 0 in Kankaku, 0 in the third app — neither app has a table or slice name containing a double space, confirmed by scanning every view's `data_source` against every known table and slice name in all three apps.
- **Scope beyond `data_source`.** The same `text_mapping` supplies `category`, `position`, `view_type`, and `ref_parent` (lines 622–629) — all read from the same lossy paste capture. `category`/`position`/`view_type` are short enum-like values with no observed double-space exposure. `ref_parent` is name-shaped and carries the identical risk, though the one anomaly found in it (`Warning` as the recorded parent for 70 Farmy views, and `User Settings` for one third-app view) traces to a different, pre-existing heuristic mis-detecting a UI banner line as a parent header — not the whitespace-collapse defect, and not examined further here.
- **Unfixable at source; recovered at use, 2026-09-06, in `get_view_table()`.** The cause above is unchanged and still accurate: `views_parser.py`'s extraction is correct given its input, the double space is destroyed before `views1.txt`/`views2.txt` exist, and no change to this module's own parsing logic can recover it. The only way to fix it at the source would be to derive `data_source` from `Application Documentation.html` instead of (or to cross-check against) the pasted text — a capture-methodology change, not a bug fix, and not made here. What changed is where the value is read: when a view's resolved `data_source` matches neither a known table nor a known slice by direct comparison, `get_view_table()` now collapses whitespace runs in the value and in every known table and slice name the same way, and treats the value as that name if EXACTLY ONE known name collapses to the same string — resolving through the slice mapping when the match is a slice, exactly as the direct path already does. Resolves all four Farmy views named above; 0 recoveries needed in Kankaku or the third app, matching the count of affected views found there.
- **The recovery depends on the collapsed form being unique — checked, and it holds in all three apps.** Every known table and slice name in Kankaku, Farmy, and the third app was collapsed and compared against every other; zero collisions found anywhere. If two distinct real names ever collapsed to the same string, the affected value would be left unresolved rather than guessed at — the function does not choose between candidates; on more than one match, or on none, it returns the value unchanged, exactly as it already did before this fix.
- **`_target_row_belongs_to_view`'s guard is unchanged and remains the correct behavior** for a name that genuinely cannot be resolved — this recovery narrows what falls into that case; it does not replace the guard, which is still reached first for the target-row side (rule 1) and still available for the view side (rule 2) if a future name defeats normalization too. Confirmed by re-parse: the guard's view-table permit (rule 2) fires 0 times in all three apps after this change, where Farmy was 3 before it.
- **The recovery is local to `get_view_table()`.** `appsheet_views.csv`'s own `data_source` and `source_table` columns still carry the collapsed, single-space string — this fix does not touch `views_parser.py` or the CSV it writes. Any other consumer of that field that compares it directly against a table or slice name, rather than going through `get_view_table()`, has the identical defect, unrepaired.

### Both saved reference parses were stale — re-cut 2026-09-06 at `c1e849e`

- **The entry this replaces described a superseded pair.** As originally written (2026-09-05) it named `20260902_131151_AppsheetFarmyApp_for_Kirk_parse` and `20260901_085853_260831_1809_Kankaku_V18_regression_reference_1c22881`, and its measurement — the `must_be_in_views`/`must_not_be_in_views` field swap that `a15021b` corrected — belongs to that pair. Those directories had already been superseded by `20260902_180352_AppsheetFarmyApp_for_Kirk_parse` and `20260902_180356_260831_1809_Kankaku_V18_baseline_parse`, adopted at `a15021b` without regeneration, and this entry was never carried forward. `RELEASE_CHECKLIST.md` section D and `CLAUDE.md` named the later pair while this entry named the earlier one. Recorded so the disagreement is not rediscovered as a contradiction: both were true when written.
- **Re-cut 2026-09-06 at `c1e849e`.** Current references: `20260906_064915_260831_1809_Kankaku_V18_baseline_parse` and `20260906_065021_AppsheetFarmyApp_for_Kirk_parse`. Both satisfy the checklist's condition — earliest file mtime inside each directory later than the newest `.py` in the repository (`views_parser.py`, 2026-09-05 21:53:57).
- **What the staleness actually cost, measured against the 180352/180356 pair.** Kankaku came back identical on every CSV: stale in principle, never misattributed in practice. Farmy differed on `navigation_edges.csv` alone, 1971 → 2063, +92 rows. That is `b68f854`'s `**auto**` mixed-slice fix, whose pre-fix investigation predicted exactly 92 new rows (81 regular, 11 via group actions). No other file differed in either app.
- Every diff from this point rests on the new pair. The control-parse-at-pre-fix-HEAD workaround the original entry prescribed is no longer needed.
