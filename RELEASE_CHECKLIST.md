...
- [ ] **Does Prominent (`Display_Prominently`) display on a dashboard view? On a calendar view?**
      ...
      **Dashboard half DONE 2026-09-07 (Kirk).** Answer: F — a `Display_Prominently` action does not display on a dashboard view. Recorded under "Established behavior" in `APPSHEET_BEHAVIOR.md` with both grounds (negative population, and a purpose-built `My website` test action), the Unknowns section no longer lists `dashboard`, and `CONSOLIDATION_PLAN.md` section 2's `dashboard` row carries the observed value. Per this item's own done-when, an F answer would join the section B form/map fall-through item — **not done as part of this documentation-only pass**; that is a code change and belongs to whoever picks up section B next, not silently folded in here. **The calendar half remains open and unscheduled** — this checklist item stays unchecked until it is done too.
...
- [x] **Map fall-through in `is_action_visible_in_view`.**
      ...
      **Fixed 2026-09-07.** `is_visible_in_view_neg` now returns `False` for `form` and `map`, no per-prominence branching, matching both `APPSHEET_BEHAVIOR.md` cells exactly. Verified BY CONSTRUCTION, not by row enumeration — 1,189 edges removed across 322 distinct action groups is past the point where individual explanation is the standard; the candidate set (direct/via_group edges whose source is form/map-type) makes scope leakage structurally impossible, confirmed at 0 leaks in any app. Corrected counts (the pre-implementation reconnaissance was computed against a now-stale, pre-`LEFT()`-fix baseline): Kankaku 174 removed, Farmy 860, third app 143; 0 added anywhere. Exactly two views newly flagged, both Farmy (`unused_system_views.csv` 99→101: `ActivityGermination_Form`, `Location_Detail` — the second a cascading, second-order loss); `potential_view_orphans.csv` unchanged at 54, since `Images_Form - Seeds`'s one edge is an event-bypassed `via_group` route (`event_type='form saved'`) the fix correctly preserves — the fix ended up more correct than its own pre-implementation gate predicted. Full detail, including the two already-known-phantom false positives in the naive reachable-set diff and the untraced `potential_action_orphans.csv` ripple, in `STATUS.md`'s matching "Recently fixed" entry.
...
- [x] **Decide: fix `view_orphan_detector.py`'s missing `CONTEXT()` handling, or
      ...
      **RESCOPED 2026-09-06, after read-only reconnaissance. The DECISION above stands; the location of the fix has moved.** The reconnaissance established that porting context evaluation into the two BFS traversals — the scope written above — would refuse zero edges in any app, by construction, since `navigation_edge_generator.py` only ever writes an edge whose condition already passed for that source view (`STATUS.md`, "Porting `check_context_conditions` into the two reachability modules unchanged would refuse zero edges, by construction"). Neither consumer reads a condition field today; that much was right. What is wrong is the assumption that the conditions are present to be read.
      *Where the real gap is:* `action_target_parser.py` never captures the `LEFT(CONTEXT("View"), N) = "prefix"` shape from an `only_if_condition`, nor `IN(CONTEXT(...), LIST(...))` in any position, so those edges reach the CSV with six empty condition fields and are indistinguishable from unconditional ones (`STATUS.md`, "`action_target_parser.py` silently drops two whole shapes of `CONTEXT()` condition"). This also SETTLES the separate open question about whether `a15021b`'s inversion handles the `NOT(IN(CONTEXT("View"), LIST(...)))` shape: it never reaches the inversion at all. That question is closed here and needs no separate work.
      *Two branches, written down so the choice is explicit rather than made inside an implementation:* **(a) upstream** — extend `action_target_parser.py` to capture the two missing shapes, populating the existing six fields, and let `check_context_conditions` do what it already does correctly; the two consumers may then need no change at all, or only the shared read the original scope described. **(b) downstream** — leave the parser as it is and build a mechanism in the consumers that re-reads `original_expression` and re-parses it more completely. These are different amounts of work with differently sized verification burdens, and the wording above did not distinguish them. Not chosen here.
      *Corrected done-when, replacing the direction-only prediction above.* A null result is a FAILURE, not a pass. Whichever branch is taken, the fix must refuse the 9 Kankaku and 36 Farmy edges named in the `STATUS.md` entry, and exactly one view must be newly flagged: Farmy's `ActivityForm - Germination`, whose 13 incoming edges all derive from one action and whose only condition-satisfying source view, `Beds Inline MyPlants Deck`, is itself unreachable from any root. Kankaku gains no newly flagged view: all three targets stay reachable via the same action's satisfiable source views. Stop and re-examine if any view is newly CLEARED, if any other view is newly flagged, or if nothing changes.
      *Two interactions to watch, both established rather than anticipated.* The 6-edge depth cap in `view_dependency_analyzer.py` is live, not theoretical: Farmy already runs 19 root-target pairs at the full six edges, and refusing edges can lengthen surviving paths, so the analyzer may report no path where the orphan detector's uncapped BFS still finds one — a new disagreement between the two modules, manufactured by this fix compounding with a pre-existing difference. And the case-matching trap recorded in the second `STATUS.md` entry above applies to any shared mechanism fed canonicalized view names.
      *Sequencing note, Kirk's call, not decided here:* `ActivityForm - Germination` flips only because its one satisfying source view is unreachable, and reachability in Farmy is currently inflated by the table-less action-to-target join recorded in `STATUS.md` ("`navigation_edge_generator.py` joins actions to targets by name alone"). Doing this fix first means explaining its removals against an edge list known to contain fabricated edges.
      **BRANCH CHOSEN 2026-09-06 (Kirk): branch (a), upstream.** Extend `action_target_parser.py` to capture the two missing shapes so the existing six condition fields are populated, and let `check_context_conditions` — which is already correct — do the refusing. Reasoning: it fixes the data rather than the reading of it, so every consumer of `navigation_edges.csv` benefits at once including the three orphan detectors never yet audited; and branch (b) would put a second expression parser in the consumers, which is precisely the duplication `CONSOLIDATION_PLAN.md` exists to undo and which has already cost this project one wrong answer in the visibility rules.
      *Sub-decision, taken with it:* a `LEFT(CONTEXT("View"), N) = "prefix"` test is EXPANDED at parse time against the known view list — the prefix resolves to the set of view names that actually begin with it, written into `must_be_in_views` in the existing format — rather than a seventh, prefix-carrying field being added. Simpler, and it keeps the field vocabulary unchanged. Known cost, accepted: the expansion is fixed at parse time, so a view renamed after the parse would need a re-parse to be reflected. That is already true of everything else in these CSVs.
      **SEQUENCED 2026-09-06: the table-less join fix goes FIRST**, before this item. `STATUS.md`, "`navigation_edge_generator.py` joins actions to targets by name alone" — Farmy's reachability is currently inflated by fabricated cross-table edges, and this item's done-when turns on one view (`ActivityForm - Germination`) flipping because its only satisfying source view is unreachable. That judgment must be made against an edge list that is not fabricating edges.

      **IN() half done, `8669c8b`/`7039486` — see `STATUS.md`'s "Recently fixed."** Everything below is the LEFT() half's own gate, corrected 2026-09-07 before implementation, from Kirk's 2026-09-06 reconnaissance plus direct re-verification against the fresh 2026-09-07 reference parses. The done-when at the "*Corrected done-when, replacing the direction-only prediction above*" bullet is written entirely in removals and does not hold as stated for the LEFT() shape: the fix also ADDS edges in Kankaku, for a reason independent of the removals — a genuinely over-restrictive `must_be_in_views` that today excludes views the corrected expansion admits. A removal-only gate would fail a correct fix. The removal figures from that bullet stand; the stop condition is superseded by the one at the end of this block.

      **LEFT() GATE, CORRECTED 2026-09-07 (Kirk's reconnaissance, re-verified directly against the fresh parses before writing this):**

      *Removals — unchanged from the original figures, re-confirmed:* Kankaku loses exactly 9 edges — `View Ref (Language flag) 3`, `View Ref (Public list) 3`, `View Ref (More dictionaries) J` (table `Options`, `only_if_condition` position, each currently 6 edges with all six condition fields empty; prefix "Options J" (N=9) expands to {Options J, Options J M, Options J S}, refusing 3 of each action's 6 source views; 3×3=9). Farmy loses exactly 36 — `Go to Germination - From MyPlants Direct Sow` (table `MyPlants`, 12 of 13 edges refused, only `Beds Inline MyPlants Deck` satisfies prefix "Beds" (N=4)) and `Nursery_Form` (table `BlankTable`, 24 of 30 edges refused across 2 targets). Farmy's `Nursery_Form 2 DELETE` and `Nursery_Form 2` produce zero edges today and must stay at zero — excluded from the 36, not folded into it. Third app: zero LEFT instances exist (its six `CONTEXT()` instances were all `IN()`-shape, already fixed); every output file must be logically identical to the fresh third-app reference, full stop.

      *Additions — not in the original done-when, Kankaku only, none in Farmy or the third app:*
      - `Session flag` (table `Settings`, `go_to_view`) gains exactly 1 edge: `Session J 2 -> Session`. This is the one LEFT instance in the whole three-app set that sits in the navigation-expression (IF/IFS) position rather than `only_if_condition`: its IFS branch `LEFT(Context("View"),9)="Session J", LINKTOVIEW("Session")` is today mis-captured by `parse_context_condition` as the single literal view name "Session J" (the regex discards `N`). Correct expansion of prefix "Session J" (N=9) is {Session J, Session J 2}. Confirmed directly against the fresh parse, not merely asserted: `Session J 2` genuinely carries `Session flag` in its `available_actions`, so this is an unconditional, mechanically-verified addition, not a live-observation-only claim.
      - `To cram from session E group` (table `Settings`, `execute_group`) MAY gain a `via_group` edge into `Cram` (through its child `To cram from session E`) — conditionally. Its `only_if_condition` is `OR(CONTEXT("View")="Session", LEFT(CONTEXT("View"),9)="Session J")`; the flat, structure-blind scan (unchanged, per the recorded Known defect) will find the plain clause and, separately, the LEFT clause if this fix recognizes it, and whether their values land unioned into one `must_be_in_views` field determines whether the OR is — by construction, not by design — reconstructed correctly. **If the union does not happen and this action's edges are unaffected, report that outcome explicitly and it is NOT a gate failure.** What IS a gate failure is silence on this action, in either direction.
      - `To cram from session E` (table `Settings`, `go_to_view`) is explicitly UNQUANTIFIED, per Kirk's own reconnaissance. Its `only_if_condition` (`LEFT(CONTEXT("View"),7)="Session"`, prefix expands to 9 views) must combine with condition fields its navigation expression's own `IF(Context("View")="Session", LINKTOVIEW("Cram"), LINKTOVIEW("Cram J"))` branch already populates by a separate mechanism (today: `must_be_in_views='Session'` on the `Cram` row, `must_not_be_in_views='Session'` on the `Cram J` row, with exactly 1 edge surviving in total — confirmed against the fresh parse: the `Cram J` row currently produces zero edges despite its populated field). Whether the two sources of restriction should intersect or union is a real implementation question this gate does not resolve. **No predicted count applies here — any movement must be reported and explained row-by-row, not measured against a number.**

      *Newly cleared views — checked directly against the fresh Kankaku parse, not assumed:* none. Every target reached by an addition above — `Session`, `Cram`, `Cram J` — already carries at least one incoming edge unrelated to any LEFT()-bearing action (`Session J -> Session`; `Session -> Cram` and `Cram J -> Cram`, both via `Session flag`; `Cram -> Cram J`, via `Session flag`), confirmed against `20260907_083159_260831_1809_Kankaku_V18_baseline_parse`. The stop-if-newly-cleared condition stands, unweakened, for any view not named here.

      *Newly flagged: exactly one, Farmy's `ActivityForm - Germination`* — re-confirmed directly against the fresh Farmy parse (`20260907_083305_AppsheetFarmyApp_for_Kirk_parse`), not carried forward from the 2026-09-06 prediction unchecked: all 13 of its incoming edges derive from `Go to Germination - From MyPlants Direct Sow`, and its only prefix-satisfying source, `Beds Inline MyPlants Deck`, is independently confirmed present in the fresh parse's `potential_view_orphans.csv` (`Ref view not reachable from any root view`). Kankaku gains no newly flagged view.

      *Two traps, both must hold as numeric invariants, not merely "not worse":*
      - `Next session (settings)` (table `Settings`, `execute_group`) — `IF(LEFT(CONTEXT("View"),20)="Session confirmation", NUMBER([Text2])=[Number], TRUE)` selects between two DATA predicates; it is not a view restriction, because the else-branch is unconditionally `TRUE`. **Its 16 edges must be BYTE-IDENTICAL before and after.** Observed live 2026-09-06: the action displays on `Session J`, which does not match the prefix — proof the action is not actually gated by it.
      - The three `set_columns` actions carrying a LEFT instance (`Stats from settings`, `Settings import backup`, `Show copied and hidden cards from Settings`) are inert by construction — `process_action`'s action-type gate excludes non-navigational action types before any condition parsing runs. **Must produce 0 `action_targets.csv` rows and 0 edges, unchanged.**

      *Explicitly out of scope, must not move as a side effect:* Farmy's `Nursery_Form 2` carries a separate, already-recorded inversion defect (`IF(CONTEXT("View")="NurseryCreating Dashboard", FALSE, ...)` parsed as `must_be_in_views='NurseryCreating Dashboard'`, the inverse of its meaning) — not fixed here, must still produce 0 edges.

      *Stop and re-examine if:* any view is newly CLEARED; any view other than `ActivityForm - Germination` is newly flagged; `Next session (settings)`'s edge count changes at all, in either direction; any of the three `set_columns` actions produces a nonzero row; Kankaku's or Farmy's removal counts differ from 9 / 36; Farmy's two zero-edge actions produce a nonzero count; the third app moves at all; or nothing changes anywhere (a null result is a FAILURE, per the standing rule above).

      **BOTH HALVES SHIPPED, 2026-09-07 — this item is done.** The rescoped upstream fix (branch (a), decided above) landed in two commits: the `IN()` half (`8669c8b`/`7039486`) and the `LEFT()` half (`9b4b549`), each verified by full re-parse of all three apps against the gate this item's own body specifies above. Full verification detail in `STATUS.md`'s two matching "Recently fixed" entries.

- [x] **Unsatisfiable column-`Show_If` / attached-action-condition pairs — the narrow mechanical half of the `Show_If` gap.**
      ...
      **Fixed 2026-09-07.** New module `unsatisfiable_condition_detector.py`, new file `potential_unsatisfiable_conditions.csv`, wired into `master_parser_and_orphan_detector.py` as Phase 14. Kankaku: exactly the 2 predicted rows. Farmy's 78 candidates hand-checked for the first time (0 found — the census had only ever cleared Kankaku's 148). Third app: 1 candidate, 0 found. Every other output file byte-identical in all three apps. The module's own docstring pins down the three spec decisions the ambiguity assessment above left for implementation (`ISBLANK`/`ISNOTBLANK`/`NOT(ISBLANK(...))` as one family; `[_THISROW].[X]` normalized to bare `[X]`; `AND(...)` decomposed into conjuncts, `OR(...)` never decomposed) plus a fourth, smaller, disclosed normalization (case-sensitive, whitespace-collapsing variable matching) that changes nothing in today's data. Full detail in `STATUS.md`'s matching "Recently fixed" entry. This was the last code item in section B.
...
- [ ] **The 120-view "other" bucket.** No longer blocked — section A closed
      ...
      **RESOLVED AS A DECISION, 2026-09-05 (Kirk, Fable planning session) — the bucket no longer exists as a single question, and no permissive-versus-restrictive default is chosen.** `CONSOLIDATION_PLAN.md` section 2 had already decomposed the 120 views by type; the pieces now go three ways:
      ...
      - **`card` (17): the one genuinely open piece, and it is not a default question either.** `APPSHEET_BEHAVIOR.md` (2026-09-05) established that card display is per-slot assignment in the Layout widget, which no prominence-keyed boolean can express. What decides its handling is one fact not yet checked: whether the export carries the Layout widget's per-slot action assignment. Recorded here as a READ-ONLY question for the execution session, with both branches written down so the answer needs no further decision: if the assignment IS in the export, parsing it is a bounded parser task of the same shape as the 2026-08-30 onClick/Layout-JSON fix; if it is NOT, card gets an entry in `APPSHEET_BEHAVIOR.md`'s scope-decisions section as an export gap and keeps its current verdicts. Either branch is POST-PUBLICATION unless the execution session finds it trivial — 17 card views do not hold up phase one.
        **ASKED AND ANSWERED, 2026-09-07, read-only, nothing implemented: the FIRST branch applies.** The assignment IS in the export — see `STATUS.md`'s "Card views: the export DOES carry per-slot action assignment" for the evidence, the per-template capacity corroboration, and the scope correction (16 of Farmy's 17 card views carry it, not 17; `MapImages list` uses no per-slot Layout at all; the third app has 4, Kankaku 0). Parsing it would be a bounded parser task of the same shape as the 2026-08-30 fix — the pairing already survives the raw JSON, and the code that already walks that tree (`views_parser.py`'s `collect_onclick_actions`) is what currently discards it into a bare sorted set. **Stays POST-PUBLICATION, per this item's own decision — not moved into phase one by this finding.**
      ...
      *Done when:* the section B form/map item and the section A dashboard/calendar item are each done per their own conditions, and the card question above has been asked and its branch recorded in `STATUS.md`. **Card leg done 2026-09-07 — asked and answered above.** **Corrected same day, after `a7c283a` landed:** section B form/map is now also done (`a7c283a`, see that item), and section A's dashboard half is also done (Prominent does not display at dashboard level, `APPSHEET_BEHAVIOR.md`). **Calendar is the only condition still open** — this bucket item stays unchecked for that reason alone.
...
- **What the parser interprets, and what it hands to the reader.** Recorded 2026-09-07, prompted by the `LEFT()` fix.

  **THE EVIDENCE THAT MADE THIS CONCRETE (established 2026-09-07, commit `9b4b549`).** The `LEFT()` fix produced two edges the app does not have — Kankaku's `To cram from session E` gains `Session J -> Cram` and `Session J 2 -> Cram`, where the app's own IF-branch logic routes those two views to `Cram J`. The cause is face (iv) of the recorded structural-blindness defect (`STATUS.md`): `finditer` accumulates over a raw `only_if_condition` string, unioning clauses the expression's structure keeps separate. This is not a fixable oversight in the regex; a correct fix needs boolean-structure parsing.

  **KIRK'S REFRAMING**, and the reason this is an architecture question rather than a bug: with AI as the reader, the parser may not need to interpret these at all. `original_expression` already carries the raw text on every edge, and a language model reading `or(context("View")="Session", left(context("View"),9)="Session J")` can get it right — including the IF-with-TRUE-else-branch case that defeated the regex — because it understands what an else-branch of `TRUE` means. A script must anticipate every shape in advance, which is exactly how this tool accumulated `LINKTOFILTEREDVIEW`, `LINKTOFORM` and two `CONTEXT()` shapes as silent gaps. A reader can say "could be this, could also be that, let's check," which is a better answer than either a wrong verdict or silence. Kirk's framing of the baseline: compared with a 12MB HTML export that supports only inefficient text search, a good-but-imperfect parse that AI can help interpret is a large step forward, and perfection is not the goal.

  **THE CONDITION THAT MAKES IT WORK (model's point, and the one that keeps the proposal honest):** "could be this, could be that" is only available when the ambiguity is VISIBLE. Today it is not — a condition field derived from an unparsed `OR` is byte-indistinguishable from one derived from a clean single clause, so a reader has no way to know which conditions to re-read. Everything in this approach depends on the parser being explicit about what it could not interpret.

  **TWO SEPARABLE PIECES (model's split, offered not decided):**
  1. **REPORT the uncertainty.** When the scan finds a `CONTEXT()` clause inside boolean structure it did not interpret, record that fact. This is detection, not interpretation — a text test for `or(`/`and(`/`if(` around the match, plus the count of clauses accumulated. Cheap, and it changes no existing value.
  2. **ACT on it.** Have the graph abstain — refuse to emit an edge whose condition came from unread structure — converting wrong rows into missing ones. This changes reachability, so it needs its own gate and verification. Not cheap.

  **DELIVERY**, which removes the schema objection: piece (1) can be a SEPARATE CSV rather than new columns on existing files, following the precedent already set for the unsatisfiable `Show_If` pair check in section B — a new file leaves every existing output byte-identical, which is the verification. Suggested shape: one row per affected action, carrying action name and source table, the raw `only_if_condition`, which structure was detected, how many clauses were accumulated, and what was written into the six fields as a result. Joinable per-action rather than per-edge. Cost: `CLAUDE.md` must tell CC to check that file before trusting a condition field, or the file is useless.

  **WHY BOTH GO TO THE PHASE-TWO ARCHITECTURE SESSION rather than into phase one. Kirk's decision, 2026-09-07.** Piece (2) meets all three conditions of the stronger-model test — hard to reverse (every downstream count moves and the orphan detectors inherit the choice), not settleable by running something (whether an uncertain edge should be emitted or withheld depends on what the tool is for), and genuinely difficult (the same abstain-versus-report trade-off the 120-view bucket met). Piece (1) is separable and cheap, but its value depends on what CC is told to do with it, and that instruction is part of the same design — building the reporting half now would mean guessing at the consuming half.
...
**ARCHITECTURE DECIDED 2026-09-07 (Kirk, Fable architecture session — the first of the three stronger-model points above). Reasoning is in the project record's Addendum 8, `260717_appsheet-ai-visibility-layer.html`; this block records only the decisions and the implementation split.** The interpret-versus-preserve question in the fourth design note is answered by *where* interpretation lives, not how much of it there is: capture and derivation are separate programs, and every derived artifact declares what it could not interpret.

1. **The Apps Script captures raw, and only raw.** Per sheet: `getFormulas()`, `getValues()` (raw values, not only display values), plus sheet names and dimensions, named ranges, data validation, conditional format rules, number formats, frozen rows/columns, protected ranges. JSON to Drive, capture date recorded. No deduplication, reference extraction, rationing, or derived file of any kind in the script.
2. **Capture all values; ration at read time.** Values in a separate file from formulas so one can be shared without the other. Clarifies, does not reverse, the 5 September rationing note: that note describes what a trace reads, not what the capture holds.
3. **Every derived artifact is built locally by a re-runnable Python tool and ships with a residue list** — formula inventory (patterns with instance ranges), schema file, column dependency graph (residue: every reference-like token not resolved — `INDIRECT`, `OFFSET`, string ranges in `IMPORTRANGE`/`QUERY`, `ARRAYFORMULA` spans), and the join below. `CLAUDE.md` must tell the reader to check the residue before trusting the artifact.
4. **App side: build the reporting half of the fourth design note, not the abstaining half.** A separate CSV, joinable per action, listing every condition field with more than one contributor or any contributor under `OR`/`IF` (trigger on "more than one contributor" alone — `STATUS.md`'s face (iv) is a conjunction). Every existing output stays byte-identical, which is the verification. Abstention is NOT built; revisit only if real use shows a flagged edge producing a wrong answer the residue file did not catch.
5. **The AppSheet-to-sheet join is its own derived file** (table→tab, column→header, by exact match), reporting ambiguities and resolving none.

*Implementation split:* Apps Script capture — Sonnet-guided Claude Code (mechanical, checkable). Local derivation tool and app-side residue CSV — Opus-guided Claude Code (reading real formulas and expressions, judging what the extractor missed; the residue CSV touches `action_target_parser.py` and needs section-B gate discipline). Routine re-runs thereafter — Sonnet. Judging the first trial runs against Addendum 5's two test cases remains the second stronger-model point and is not implementation.

*Open, recorded in Addendum 8, not decided:* which state (full / sampled / boundary) Kirk's one important external source gets; whether bound Apps Script projects and triggers are captured as source or declared as a boundary; whether a whole-workbook capture fits Google's per-run execution limit (fallback is per-sheet runs, never sampling); whether display values are kept beside raw values.
