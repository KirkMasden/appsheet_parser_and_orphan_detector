...
**Superseded 2026-09-05.** The premise was wrong: there is no designation gate on map
views, because there is no designation control at all. The map cell IS a boolean, and a
restrictive one — see `APPSHEET_BEHAVIOR.md`. The observation that this legend lacks a
needed cell type has instead migrated to `card`, where display depends on a layout choice
and a per-slot assignment that no prominence-keyed boolean can express.
